...
if apiVersStr[0:4] == "2015":
    import shiboken
    print "Maya Version 2015 : ", apiVersStr[0:4]
if apiVersStr[0:4] == "2018":
    import shiboken2
    print "Maya Version 2018 : ", apiVersStr[0:4]
...
class KmaxWin(QtWidgets.QWidget, kmaxUi2.Ui_kmaxToolBar):  # QtWidgets?
    # class KmaxWin(QtGui.QWidgets, kmaxUi2.Ui_kmaxToolBar):
    '''
    Mon outil custom de ouf

    :param QtWidgets.QWidget: QWidget import
    :type QtWidgets.QWidget:
    :param kmaxUi2.Ui_kmaxToolBar: Ui_kmaxToolBar import
    :type kmaxUi2.Ui_kmaxToolBar:
    '''
    ...
    def initIcon(self):
        ...
        # icons interface
        self.iconLockOn = QtWidgets.QIcon()
        self.iconLockOn.addPixmap(QtWidgets.QPixmap(self.target + "lockOn.png"), QtWidgets.QIcon.Normal, QtWidgets.QIcon.Off)
        ...
        self.iconLockOff = QtWidgets.QIcon()
        self.iconLockOff.addPixmap(QtWidgets.QPixmap(self.target + "lockOff.png"), QtWidgets.QIcon.Normal, QtWidgets.QIcon.Off)

        '''
        pixmapTool = QtWidgets.QPixmap(self.target + "aselect.png")
        #pixmapToolScaled = QtWidgets.QPixmap.scaled(32)
        #pixmapTool = pixmapTool.scaled(28, 28)#, QtWidgets.QPixmap.FastTransformation)
        self.lb_tool.setPixmap(pixmapTool)#.scaled(32, 32, QtWidgets.QPixmap.IgnoreAspectRatio, QtWidgets.QPixmap.FastTransformation))
        '''
        ...
        for iconName, iconFilename, btn, typeBtn in buttonIconItems:
            if typeBtn == "bouton":
                icon = QtWidgets.QIcon(name=iconName)
                icon.addPixmap(QtWidgets.QPixmap(self.target + iconFilename), QtWidgets.QIcon.Normal, QtWidgets.QIcon.Off)
                btn.setIcon(icon)

            if typeBtn == "label":
                pixmapName = QtWidgets.QPixmap(self.target + iconFilename)
                btn.setPixmap(pixmapName)

    def setIconTool(self, toolType):
        pixmapTool = QtWidgets.QPixmap(self.target + toolType + ".png")
...
def launchUi():
    ...
    if mc.dockControl(sDockName, q=True, ex=True):
        ...
        myUi = qDock.findChild(QtWidgets, "kmaxToolBar")  # QtWidgets
        myUi.close()
        mc.deleteUI(sDockName)
    ...
    '''
    if mc.window("kmaxTmpWin", query=True, exists=True):
        print "kmaxTmpWin exist !"
        # myWindow = mc.window("kmaxTmpWin")
        mc.deleteUI("kmaxTmpWin")
