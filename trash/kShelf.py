...
class kShelfBar():
    def __init__(self):
        ...
        self.iconSize = 22
        self.separatorWidth = 16
        self.initPath()
        self.createShelfUI()

        WSCN = "kShelf3"
        if mc.workspaceControl(WSCN, q=True, exists=True):
            print ">>> le workspace : " + WSCN + " existe deja !!!"
            mc.workspaceControl(WSCN, edit=True, close=True)
            mc.workspaceControl(WSCN, close=True, edit=True)
            mc.deleteUI(WSCN, control=True)
        else:
            print ">>> il n'existe pas !!"
        mc.workspaceControl(WSCN, floating=False, uiScript='self.createShelfUI()')
        mc.workspaceControl(WSCN, edit=True, retain=False, initialHeight=self.iconSize, dockToControl=("Shelf", "top"))

    def createShelfUI(self):
        #windowName = "kShelfTop"
        #toolName = "toolShelfTop"
        #workspaceControlName = "kShelfWorkspace"
        ...
        mc.rowColumnLayout(numberOfRows=1, rowHeight=(1, self.iconSize))

        for btnName, imgFileName, imgHltFileName, btnAnnotation, btnCommand in allButtons:

            if btnName == "bt_h01":
                self.bt_h01 = mc.iconTextButton(style='iconOnly', image1=self.target + "digit8.png", width=22, enable=0)
            if btnName == "bt_h02":
                self.bt_h02 = mc.iconTextButton(style='iconOnly', image1=self.target + "digit8.png", width=22, enable=0)
            if btnName == "bt_m01":
                self.bt_m01 = mc.iconTextButton(style='iconOnly', image1=self.target + "digit8.png", width=22, enable=0)
            if btnName == "bt_m02":
                self.bt_m02 = mc.iconTextButton(style='iconOnly', image1=self.target + "digit8.png", width=22, enable=0)

            if imgFileName != "digit8.png" :
                btnName = mc.iconTextButton(style='iconOnly',
                                            image1=self.target + imgFileName,
    ...
    def initPath(self):
        ...
        for item in path_list:
            self.target += item + '/'
        '''
        self.target = "E:/kTools/icons/"
        print ">> icons path :", self.target
