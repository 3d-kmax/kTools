...
class kShelfBar():
    def __init__(self):
        ...
        self.scaleIcon = 22
        self.scaleSeparator = 16
        self.initPath()
        self.kShelfUI()

        workspaceControlName = "kShelf"

        if mc.workspaceControl(workspaceControlName, q=True, exists=True):
            print ">> le workspace : " + workspaceControlName + " existe deja !!"
            mc.workspaceControl(workspaceControlName, edit=True, close=True)
        else:
            print ">> il n'existe pas !!"
        mc.workspaceControl(workspaceControlName, retain=False, floating=True, uiScript="kShelfUI()")
    ...
    def kShelfUI(self):
        ...
        mc.rowColumnLayout(numberOfRows=1, rowHeight=(1, self.scaleIcon))
